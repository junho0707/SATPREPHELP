...
def extract_mathjax_equation(element: Locator, page: Page, images_dir: str,
    ...
    Returns:
        FigureData with:
        - text_content: alttext from mjx-container
        - structured_data: {alttext, mathml (if available)}
        - image_path: screenshot of the equation
    """
    figure = FigureData(
    ...
    try:
        # Check if element IS the mjx-container or contains one
...
def extract_graph_svg(element: Locator, page: Page, images_dir: str,
    ...
    Returns:
        FigureData with:
        - text_content: aria-label description
        - structured_data: {aria_label, viewBox, has_axes, etc.}
        - image_path: screenshot of the graph
    """
    ...
    try:
        # Check if element IS the SVG or contains one
        tag = get_tag_name(element)
        if tag == "svg":
            svg = element
        else:
        ...
        # Check for axis labels (use textContent for SVG text elements)
        axis_labels = svg.locator("text")
        if axis_labels.count() > 0:
            labels = []
            for i in range(min(axis_labels.count(), 10)):  # Limit to prevent slowdown
                try:
...
def extract_equation_image(element: Locator, page: Page, images_dir: str,
    ...
    Returns:
        FigureData with:
        - text_content: alt attribute
        - structured_data: {alt}
        - image_path: saved PNG file from base64
    """
    figure = FigureData(
    ...
    try:
        # Check if element IS the img or contains one
        tag = get_tag_name(element)
        if tag == "img":
            img = element
        else:
...
def extract_table_html(element: Locator, page: Page, images_dir: str,
    ...
    Returns:
        FigureData with:
        - text_content: reconstructed table as text
        - structured_data: {headers: [...], rows: [[...]]}
        - image_path: screenshot of table
    """
    figure = FigureData(
    ...
    try:
        # Check if element IS the table or contains one
        tag = get_tag_name(element)
        if tag == "table":
            table = element
        else:
        ...
        for i in range(header_cells.count()):
            ...
            # Try to get MathJax alttext, fallback to text content
        ...
        # Extract rows (td elements)
        body_rows = table.locator("tbody tr")
        start_idx = 1 if headers else 0  # Skip first row if it was headers
...
def extract_table_image(element: Locator, page: Page, images_dir: str,
    ...
    Returns:
        FigureData with:
        - text_content: combined alt texts
        - structured_data: {rows: [{text, image_path}, ...]}
        - image_path: screenshot of full table
    """
    figure = FigureData(
...
def extract_mixed_content(element: Locator, page: Page, images_dir: str,
    ...
    Returns:
        FigureData with:
        - text_content: text with equations replaced by alttext
        - structured_data: {equations: [alttext, ...], has_inline_equations: true}
        - image_path: screenshot of full content block
    """
    figure = FigureData(
...
def extract_figure(element: Locator, page: Page, images_dir: str,
    ...
    if extractor:
        ...
        # Fallback: just screenshot
        figure = FigureData(
            figure_type=FigureType.UNKNOWN,
            position=position,
            text_content=get_element_text(element),  # Safe for SVG
        )
    ...
    figure.figure_index = index  # Set index for {{FIG_N}} mapping
    return figure
...
def extract_all_figures_from_section(page: Page, section_selector: str,
    ...
    """
    Extract all figures from a section (prompt, question, choices, rationale).
    Uses proper deduplication to avoid capturing the same content multiple times.
    """
    figures = []
    ...
    # Selectors in priority order - containers first, then leaf elements
    # Process containers first so their children are skipped
    figure_selectors = [
        "figure",                           # Explicit figure elements (highest priority)
        "table:has(mjx-container)",         # Tables with MathJax
        "table:has(img.math-img)",          # Tables with images
        "mjx-container",                    # MathJax equations (BEFORE svg - mjx contains svg!)
        "svg[role='img'][aria-label]",      # Only SVG graphs with aria-label (actual graphs)
        "img.math-img",                     # Equation images
    ]

    index = start_index
    processed_elements = []  # Store JS handles for containment checks
    ...
    for selector in figure_selectors:
        ...
        for i in range(elements.count()):
            ...
            # Check if this element is inside or contains an already-processed element
            try:
                is_duplicate = element.evaluate("""(el, processedIds) => {
                    // Check if this element is already processed (by checking a data attribute we set)
                    ...
                    // Check if this element contains any already-processed elements
    ...
    # Clean up the data attributes we added
    try:
        section.evaluate("""el => {
...
def extract_question_data_enhanced(page: Page, question_id: str, images_dir: str) -> dict:
    ...
    question_data = {
        ...
        "correct_answer": "",
        "rationale": "",
        "figures": [],  # Enhanced: list of FigureData dicts
    }

    # 1) Info table (assessment, section, domain, skill, difficulty)
    ...
    # 4) Answer choices (with placeholders for each choice)
    ...
    # Extract figures from all choices
